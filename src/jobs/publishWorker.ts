import { WORKER_MAX_ATTEMPTS, WORKER_BACKOFF_SECONDS } from "../config.js";
...
async function tickPublishQueue(): Promise<void> {
  ...
  const row = getNextScheduledPost(now);
  if (!row) return;

  const attempt = row.attempts + 1;
  setScheduledAttempts(row.id, attempt);

  const result = await postTweetViaUi(row.body);

  if (result.ok) {
    markScheduledPosted(row.id);
    if (result.tweetUrl) {
      updateScheduledTweetUrl(row.id, result.tweetUrl);
    }
    if (row.draft_id) {
      updateDraftStatus(row.draft_id, "posted", now);
    }
  } else {
    const err = result.error ?? "Erro desconhecido";

    if (attempt >= WORKER_MAX_ATTEMPTS) {
      markScheduledFailed(row.id, `${err} (após ${WORKER_MAX_ATTEMPTS} tentativas)`);
    } else {
      const backoffSec = WORKER_BACKOFF_SECONDS[attempt - 1] ?? 600;
      const nextRun = new Date(Date.now() + backoffSec * 1000).toISOString();
      reschedulePost(row.id, nextRun, `Tentativa ${attempt}/${WORKER_MAX_ATTEMPTS}: ${err}`);
    }
  }
